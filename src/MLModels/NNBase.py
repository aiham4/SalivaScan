...
class NNBase:
    def __init__(self, name):
        ...
        self.folds = 5
        self.activation_functions = [None, nn.Softmax(dim=1), nn.Sigmoid(), nn.ReLU()]
    ...
    def train(self, model: nn.Module, X_train_val, y_train_val) -> TransformerMixin | None:
        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters())

        X_train, X_val, y_train, y_val = train_test_split(
            X_train_val, y_train_val, test_size=2/8, random_state=42
        )
    ...
    def get_kfold_mse(self, model: nn.Module, X, y):
        skf = StratifiedKFold(
            n_splits=self.folds,
            shuffle=True,
            random_state=42
    ...
    def objective(self, trial, X, y):
        self.trial = trial
        mse = self.get_kfold_mse(self.gen_model().to(self.device), X, y)
        self.trial = None
        return mse
    
    def optimize(self, X, y, n_trials=None):
        def objective(trial):
            return self.objective(trial, X, y)
    ...
    def confusion_matrix(self, model: nn.Module, X, y):
        skf = StratifiedKFold(
            n_splits=self.folds,
            shuffle=True,
            random_state=42
